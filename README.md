...
![CI Status](https://img.shields.io/badge/github-repo-lgreen?logo=github)
...
* **1. Generate new logs locally**
    ...
    ```
* **2. Run analysis without rebuilding**: Instead of rebuilding the image, mount your local `mock_logs.json` directly into the container using a Volume:
    * **In Powershell**:
    ```bash
    docker run --rm -v ${PWD}/mock_logs.json:/app/mock_logs.json cloud-security-analyzer
    ```
